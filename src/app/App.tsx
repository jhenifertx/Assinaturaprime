...
<div style="font-family: 'Titillium Web', Tahoma, Arial, sans-serif; max-width: 650px; color: #002753;">
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-['Poppins',sans-serif] selection:bg-[#f47920] selection:text-white">
      {/* Header */}
      <header className="bg-gradient-to-r from-[#002753] via-[#003a6f] to-[#004a8f] text-white shadow-xl">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-8 lg:px-16 py-6 sm:py-8">
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="bg-white/10 p-2 sm:p-3 rounded-2xl backdrop-blur-sm shadow-inner transition-transform hover:scale-105 duration-300">
              <img src={imgPrimeControl} alt="Prime Control" className="h-8 sm:h-10 lg:h-12 w-auto" />
            </div>
            <div className="h-10 sm:h-12 w-px bg-white/20 hidden sm:block mx-2" />
            <div className="font-['Titillium_Web',sans-serif]">
              ...
              <p className="text-blue-100/70 text-xs sm:text-sm font-semibold tracking-wider uppercase">
                Prime Control
              </p>
            </div>
          </div>
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-md">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-8 lg:px-16 py-4">
          <div className="flex items-center justify-between font-['Titillium_Web',sans-serif]">
            {/* Step 1 */}
            <div className="flex items-center gap-3 group">
              <div className={`flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl font-bold text-sm transition-all duration-500 scale-100 group-hover:scale-105 ${
                currentStep >= 1 ? 'bg-[#f47920] text-white shadow-lg shadow-orange-500/20' : 'bg-slate-100 text-slate-400'
              }`}>
                {currentStep > 1 ? <CheckIcon size={18} strokeWidth={3} className="animate-in zoom-in-50 duration-300" /> : '1'}
              </div>
              <span className={`font-bold text-xs sm:text-sm hidden sm:inline tracking-wider transition-colors ${currentStep >= 1 ? 'text-[#002753]' : 'text-slate-400'}`}>DADOS</span>
            </div>

            <div className="flex-1 h-1 bg-slate-100 mx-4 rounded-full overflow-hidden">
              <div className={`h-full transition-all duration-700 ease-in-out ${currentStep >= 2 ? 'bg-[#f47920] w-full' : 'bg-slate-100 w-0'}`} />
            </div>

            {/* Step 2 */}
            <div className="flex items-center gap-3 group">
              <div className={`flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl font-bold text-sm transition-all duration-500 scale-100 group-hover:scale-105 ${
                currentStep >= 2 ? 'bg-[#f47920] text-white shadow-lg shadow-orange-500/20' : 'bg-slate-100 text-slate-400'
              }`}>
                {currentStep > 2 ? <CheckIcon size={18} strokeWidth={3} className="animate-in zoom-in-50 duration-300" /> : '2'}
              </div>
              <span className={`font-bold text-xs sm:text-sm hidden sm:inline tracking-wider transition-colors ${currentStep >= 2 ? 'text-[#002753]' : 'text-slate-400'}`}>GERAR</span>
            </div>

            <div className="flex-1 h-1 bg-slate-100 mx-4 rounded-full overflow-hidden">
              <div className={`h-full transition-all duration-700 ease-in-out ${currentStep >= 3 ? 'bg-[#f47920] w-full' : 'bg-slate-100 w-0'}`} />
            </div>

            {/* Step 3 */}
            <div className="flex items-center gap-3 group">
              <div className={`flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl font-bold text-sm transition-all duration-500 scale-100 group-hover:scale-105 ${
                currentStep >= 3 ? 'bg-[#f47920] text-white shadow-lg shadow-orange-500/20' : 'bg-slate-100 text-slate-400'
              }`}>
                {currentStep >= 3 ? <CheckIcon size={18} strokeWidth={3} className="animate-in zoom-in-50 duration-300" /> : '3'}
              </div>
              <span className={`font-bold text-xs sm:text-sm hidden sm:inline tracking-wider transition-colors ${currentStep >= 3 ? 'text-[#002753]' : 'text-slate-400'}`}>COPIAR</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Main Content */}
      <main className="max-w-[1400px] mx-auto px-4 sm:px-8 lg:px-16 py-8 lg:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          {/* Form Column */}
          <div className="lg:col-span-5 order-2 lg:order-1">
            <div className="bg-white rounded-[2rem] shadow-xl border-l-[6px] border-orange-500 p-8 sm:p-10 lg:sticky lg:top-32 transition-shadow hover:shadow-2xl hover:shadow-orange-500/5 duration-500">
              <div className="mb-10 font-['Titillium_Web',sans-serif]">
                <h2 className="text-2xl sm:text-3xl text-[#002753] font-bold mb-2 tracking-tight">
                  Gere sua assinatura
                </h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6 font-['Poppins',sans-serif]">
                {/* Nome e sobrenome */}
                <div>
                  <label htmlFor="nome" className="block text-slate-700 font-semibold mb-1.5 text-sm">
                  ...
                  <div className="relative group">
                    <input
                      ...
                      className={`w-full px-5 py-3.5 bg-white border rounded-xl focus:outline-none focus:ring-4 focus:ring-orange-500/5 transition-all text-sm text-slate-700 placeholder:text-slate-400 placeholder:font-normal font-medium ${
                ...
                {/* Cargo */}
                <div>
                  <label htmlFor="cargo" className="block text-slate-700 font-semibold mb-1.5 text-sm">
                ...
                <div className="grid grid-cols-12 gap-4">
                  <div className="col-span-3">
                    <label className="block text-slate-700 font-semibold mb-1.5 text-sm">DDD</label>
                    <input
                      id="celularDDD"
                      type="text"
                      ...
                      className={`w-full px-2 py-3.5 bg-white border rounded-xl focus:outline-none focus:ring-4 focus:ring-orange-500/5 transition-all text-sm text-slate-700 placeholder:text-slate-400 placeholder:font-normal font-bold text-center ${
                  ...
                  <div className="col-span-9">
                    <label className="block text-slate-700 font-semibold mb-1.5 text-sm">Celular</label>
                    <input
                      id="celularNumero"
                      type="text"
                ...
                <div>
                  ...
                  {showMoreInfo && (
                    <div className="mt-4 p-5 bg-slate-50/50 rounded-2xl space-y-4 border border-slate-100 animate-in slide-in-from-top-2 fade-in duration-300">
                      <div className="grid grid-cols-12 gap-3">
                        <div className="col-span-3">
                          <label className="block text-slate-500 font-bold mb-1 text-[10px] uppercase">DDD</label>
                          <input
                            id="telefoneDDD"
                            type="text"
                            value={formData.telefoneDDD}
                            onChange={handleChange}
                            className="w-full px-2 py-2.5 bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-orange-400 transition-all text-slate-700 placeholder:text-slate-400 placeholder:font-normal font-bold text-center text-sm"
                            placeholder="00"
                            maxLength={2}
                          />
                        </div>
                        <div className="col-span-9">
                          <label className="block text-slate-500 font-bold mb-1 text-[10px] uppercase">Telefone Fixo</label>
                          <input
                            id="telefoneNumero"
                            type="text"
                            value={formData.telefoneNumero}
                            onChange={handleChange}
                            className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:outline-none focus:border-orange-400 transition-all text-slate-700 placeholder:text-slate-400 placeholder:font-normal font-medium text-sm"
                ...
                <button
                  ...
                  className={`w-full py-4.5 rounded-2xl font-bold uppercase tracking-widest text-sm transition-all shadow-lg font-['Titillium_Web',sans-serif] flex items-center justify-center gap-3 active:scale-95 ${
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 space-y-8">
            {currentStep === 1 && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                {/* Video Card */}
                <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-slate-100 transition-shadow hover:shadow-2xl duration-500 font-['Poppins',sans-serif]">
                  <div className="p-8 flex items-center justify-between font-['Titillium_Web',sans-serif]">
                    <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-[#002753] rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden group transition-all hover:shadow-2xl hover:shadow-[#002753]/20 duration-500">
                    <div className="relative z-10 font-['Titillium_Web',sans-serif]">
                      <div className="p-3 bg-white/10 rounded-2xl inline-block mb-6 transform group-hover:rotate-12 transition-transform duration-500">
                        <DownloadIcon size={24} className="opacity-80" />
                      </div>
                      <h3 className="text-xl font-bold mb-2">Manuais de Apoio</h3>
                      <p className="text-blue-100/60 text-sm mb-6 leading-relaxed font-['Poppins',sans-serif]">Baixe o guia oficial para aplicar sua assinatura de e-mail corretamente.</p>
                      <button className="py-3 px-6 bg-white text-[#002753] rounded-xl font-bold hover:bg-[#f47920] hover:text-white transition-all transform active:scale-95 text-xs tracking-wider shadow-lg">
                        BAIXAR TUTORIAL
                      </button>
                    </div>
                    <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-white/5 rounded-full blur-3xl transform group-hover:scale-150 transition-transform duration-1000" />
                  </div>

                  <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 p-8 flex flex-col items-center justify-center text-center transition-colors hover:border-orange-300 group duration-500">
                    <div className="bg-slate-50 p-6 rounded-full mb-4 group-hover:bg-orange-50 transition-colors duration-500">
                      <MailIcon size={32} className="text-slate-200 group-hover:text-orange-300 transition-colors" />
                    </div>
                    <h3 className="text-lg font-bold text-slate-700 font-['Titillium_Web',sans-serif] uppercase tracking-tighter">Preview</h3>
                    <p className="text-slate-400 text-xs font-medium max-w-[200px]">Preencha o formulário para visualizar o resultado em tempo real.</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {currentStep >= 2 && (
              <div className="space-y-8 animate-in slide-in-from-right fade-in duration-500 font-['Poppins',sans-serif]">
                {/* Success Banner */}
                ...
                {/* Final Preview */}
                <div className="bg-white rounded-[2rem] shadow-2xl p-10 border border-slate-100 relative group">
                  <h2 className="text-2xl font-bold text-[#002753] font-['Titillium_Web',sans-serif] mb-8 uppercase tracking-tighter">
                    Resultado Final
                  </h2>

                  <div className="bg-slate-50 rounded-3xl p-8 mb-8 border border-slate-100 flex items-center justify-center shadow-inner transition-colors group-hover:bg-slate-100 duration-500">
                    <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-50 w-full overflow-x-auto transform transition-transform group-hover:scale-[1.02] duration-500">
                      <div 
                ...
                <button
                  ...
                  className="w-full text-slate-400 hover:text-orange-500 transition-all font-bold text-xs uppercase tracking-widest py-4 hover:tracking-[0.2em] transform duration-300"
                >
